...
# Configuración inicial para usar el tema actual del sistema
...
class MediaApp:
  ...
  def __init__(self, root):
    ...
    # 1. BARRA SUPERIOR (Detalles y Selección de Tema)
    ...
    # PANEL IZQUIERDO (Selector y Listas)
    ...
    self.video_sidebar = ctk.CTkFrame(
        self.sidebar_dynamic, fg_color="transparent"
    )
    self.video_sidebar.grid(row=0, column=0, sticky="nsew")

    self.photo_sidebar = ctk.CTkFrame(
        self.sidebar_dynamic, fg_color="transparent"
    )
    self.photo_sidebar.grid(row=0, column=0, sticky="nsew")

    # PANEL DERECHO (Visores multimedia y controles)
    ...
    # Carga ligera inmediata de Interfaces UI base
    self.init_video_ui()
    self.init_photo_ui()
    self.update_listbox_colors(ctk.get_appearance_mode())
    self.switch_view("🎬 Vídeos")

    # 🚀 CARGA DIFERIDA CRÍTICA: VLC se importa y arranca tras pintar la ventana
    self.root.after(20, self.init_heavy_components)

  def init_heavy_components(self):
    """Inicializa VLC y temporizadores de forma diferida para arranque instantáneo."""
    global vlc
    import vlc
  ...
  def update_listbox_colors(self, mode: str):
    ...
    for lb in [self.v_listbox, self.p_listbox]:
      lb.config(
          bg=lb_bg,
          fg=lb_fg,
  ...
  # ---------------------------------------------------------
  # INTERFAZ Y CONTROLES DE VÍDEOS
  # ---------------------------------------------------------
  def init_video_ui(self):
    btn_load = ctk.CTkButton(
        self.video_sidebar, text="📂 Cargar Vídeos", command=self.load_videos
    )
    btn_load.pack(side="top", fill="x", padx=10, pady=(10, 5))

    self.v_listbox = tk.Listbox(
        self.video_sidebar, width=32, selectmode=tk.SINGLE
    )
    self.v_listbox.pack(
    ...
    self.video_frame = tk.Frame(self.video_right, bg="black")
    self.video_frame.pack(fill="both", expand=True, padx=5, pady=5)

    self.seek_bar = ctk.CTkSlider(
        self.video_right, from_=0, to=100, command=self.on_seek
    )
    self.seek_bar.set(0)
    ...
    controls_frame = ctk.CTkFrame(self.video_right, fg_color="transparent")
  ...
  # ---------------------------------------------------------
  # INTERFAZ Y CONTROLES DE FOTOS
  # ---------------------------------------------------------
  def init_photo_ui(self):
    btn_load_p = ctk.CTkButton(
        self.photo_sidebar, text="🖼️ Cargar Fotos", command=self.load_photos
    )
    btn_load_p.pack(side="top", fill="x", padx=10, pady=(10, 5))

    self.p_listbox = tk.Listbox(
        self.photo_sidebar, width=32, selectmode=tk.SINGLE
    )
    self.p_listbox.pack(
        side="top", fill="both", expand=True, padx=10, pady=(0, 10)
    )
    ...
    self.canvas = tk.Canvas(self.photo_right, bg="black", cursor="hand2")
    self.canvas.pack(fill="both", expand=True, padx=5, pady=5)

    self.context_menu = tk.Menu(self.root, tearoff=0)
    self.context_menu.add_command(
        label="⏭ Siguiente Foto", command=self.next_photo
    )
    self.context_menu.add_command(
        label="⏮ Anterior Foto", command=self.prev_photo
    )
    ...
    self.context_menu.add_separator()
    self.context_menu.add_command(
        label="🔍 Zoom +", command=lambda: self.adjust_zoom_by(1.15)
    )
    self.context_menu.add_command(
        label="🔍 Zoom -", command=lambda: self.adjust_zoom_by(1 / 1.15)
    )
    ...
    p_controls_frame = ctk.CTkFrame(self.photo_right, fg_color="transparent")
  ...
  # ---------------------------------------------------------
  # MÉTODOS DE SOPORTE Y LÓGICA (Sin cambios funcionales)
  # ---------------------------------------------------------
  ...
  def rename_selected_video(self):
    ...
    new_name = simpledialog.askstring(
        "Cambiar Nombre",
        "Ingrese el nuevo nombre para el video:",
        initialvalue=name_only,
        parent=self.root,
    )
    if new_name:
      ...
      try:
        ...
        if idx == self.current_index:
          ...
          self.update_file_details(new_path, "video")
  ...
  def rename_selected_photo(self):
    ...
    new_name = simpledialog.askstring(
        "Cambiar Nombre",
        "Ingrese el nuevo nombre para la foto:",
        initialvalue=name_only,
        parent=self.root,
    )
    if new_name:
    ...
    try:
      ...
      if isinstance(cb, list):
        for f in cb:
          if (
              f.lower().endswith(
                  (".png", ".jpg", ".jpeg", ".bmp", ".webp", ".heic")
              )
              and f not in self.photo_playlist
          ):
        ...
        if (
            ...
            and text not in self.photo_playlist
        ):
  ...
  def adjust_volume(self, delta):
    if hasattr(self, "player"):
      self.player.audio_set_volume(
          max(0, min(100, self.player.audio_get_volume() + delta))
      )
  ...
  def show_photo(self, idx):
    if 0 <= idx < len(self.photo_playlist):
      ...
      cw, ch = max(500, self.canvas.winfo_width()), max(
          320, self.canvas.winfo_height()
      )
  ...
  def zoom_image(self, e):
    if hasattr(self, "original_img"):
      if (hasattr(e, "delta") and e.delta > 0) or (
          hasattr(e, "num") and e.num == 4
      ):
        self.adjust_zoom_by(1.15)
      elif (hasattr(e, "delta") and e.delta < 0) or (
          hasattr(e, "num") and e.num == 5
      ):
  ...
  def update_zoomed_image(self):
    if hasattr(self, "original_img"):
      nw, nh = int(self.base_w * self.zoom_scale), int(
          self.base_h * self.zoom_scale
      )
      if nw >= 10 and nh >= 10:
  ...
  def next_photo(self):
    if self.photo_playlist:
      self.show_photo(
          (self.current_p_index + 1) % len(self.photo_playlist)
      )
